...
//env.allowedActions(s)
// create the TD agent
var spec = {}; // see full options on DQN page
spec.update = 'qlearn'; // qlearn | sarsa
spec.gamma = 0.9; // discount factor, [0, 1)
spec.epsilon = 0.7; // initial epsilon for epsilon-greedy policy, [0, 1)
spec.alpha = 0.01; // value function learning rate
spec.experience_add_every = 1; // number of time steps before we add another experience to replay memory
spec.experience_size = 200; // size of experience replay memory
spec.learning_steps_per_iteration = 400;
spec.tderror_clamp = 1.0; // for robustness
spec.num_hidden_units = 100 // number of neurons in hidden layer
spec.smooth_policy_update = true;
// eligibility traces
//spec.lambda = 10; // eligibility trace decay. 0 = no eligibility traces used
//spec.planN = 10; // number of planning steps per learning iteration (0 = no planning)
...
var trevel = {
    ...
    takeAction: function()
    {
        ...
        trevel.lowBetBinomial = (trevel.BinomialByMultiplication(_numberOfSuccesses, _numberOfTrials, trevel.lowBetProbability)).toFixed(18);
        trevel.highBetBinomial = (trevel.BinomialByMultiplication(_numberOfSuccesses, _numberOfTrials, trevel.highBetProbability)).toFixed(18);

        trevel.action = agent.act(trevel.experienceArry);
        if(trevel.action == 1){
            trevel._nextBet = "HB";
        }
        else if(trevel.action == 0){
            trevel._nextBet = "LB";
        }

        trevel.experienceArry = (trevel.action, trevel.highBetBinomial, trevel.lowBetBinomial, trevel.reward );
        agent.learn(trevel.reward);
        trevel.totalReward = trevel.totalReward + trevel.reward;
        console.log("Current Balance: " + parseFloat($('#balance').html()) + " Profit: " + trevel.profit + " Win percentage: " + (trevel.wins / (trevel.wins + trevel.looses)*100).toFixed(2) + "% ");
    },
};
